...
contract OntologyAttestation {
    ...
    function attest(string calldata ontologyIri, string calldata versionIri, string calldata name, int hash, int[] calldata classHashes) public {
        require(bytes(ontologyIri).length > 0, "IRI required");
        require(bytes(name).length > 0, "Name required");
        require(hash > 0, "Ontology hash required");
    ...
    function verify(string calldata ontologyIri, string calldata versionIri, int hash) public view 
